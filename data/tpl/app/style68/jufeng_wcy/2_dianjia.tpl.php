<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('dianjiaheader', TEMPLATE_INCLUDEPATH)) : (include template('dianjiaheader', TEMPLATE_INCLUDEPATH));?>
			...
			<li class="shopli">
             ...
             <?php  if($item['enabled'] == "0") { ?>      
                     <div><span class="label label-default">休息中</span></div>
                      <?php  } ?>
                       <div>&nbsp;&nbsp;</div>
                    <div><button type="button" class="btn btn-sm btn-info" onclick="javascript:document.location.href='http://apis.map.qq.com/uri/v1/geocoder?coord=<?php  echo $item['loc_x'];?>,<?php  echo $item['loc_y'];?>';"><i class="glyphicon glyphicon-map-marker"></i>&nbsp;<?php  echo $item['dist'];?></button></div>
