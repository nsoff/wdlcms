...
<div class="clearfix template">
	<?php  if($do == 'installed') { ?>
		...
		<nav role="navigation" class="navbar navbar-default" style="min-height:40px;">
			<div class="container-fluid">
				...
				<ul class="nav navbar-nav nav-btns">
					<li <?php  if($_GPC['type'] == 'all' || empty($_GPC['type'])) { ?> class="active" <?php  } ?>>
						<a href="<?php  echo url('extension/theme/installed', array('type' => 'all'));?>" style="padding-top: 10px;padding-bottom: 10px;">全部</a>
					</li>
					<?php  if(is_array($temtypes)) { foreach($temtypes as $type) { ?>
						<li <?php  if($_GPC['type'] == $type['name']) { ?> class="active" <?php  } ?>>
							<a href="<?php  echo url('extension/theme/installed', array('type' => $type['name']));?>" style="padding-top: 10px;padding-bottom: 10px;"><?php  echo $type['title'];?></a>
						</li>
					<?php  } } ?>
				</ul>
			</div>
					...
					try {
						...
						$('.upgrade-label').each(function(){
							...
							if(ret[n]) {
								...
								if(ret[n].version > v) {
									upgrade.push(n);
									$(this).html('<div class="img-rounded update-bg"></div><div class="update-div"><a href="<?php  echo url('extension/theme/upgrade')?>&templateid=' + n + '" class="btn-danger img-circle update-btn" title="来自微动力云服务更新">更新</a></div>');
								}
							}
						});
						if(upgrade.length > 0) {
		...
		<div class="form clearfix" ng-controller="listInstallThemes">
			<div class="item" id="templates-{{m.name}}" ng-repeat="m in templates">
				<div class="title">
					...
					<img src="http://addons.wdlcms.com/resource/attachment/{{m.logo}}" class="img-rounded" />
				</div>
				<div class="btn-group  btn-group-justified">
