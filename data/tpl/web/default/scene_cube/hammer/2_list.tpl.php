<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('list_header', TEMPLATE_INCLUDEPATH)) : (include template('list_header', TEMPLATE_INCLUDEPATH));?>
<div id="main">
        ...
        <div class="row-fluid">
            <div class="span12">
                <div class="box">
                    <div class="box-title">
                        <div class="pull-left">
                            <h3><i class="icon-table"></i>微场景画面<small><?php  echo $app['title'];?></h3>
                        </div>
                        ...
                        <div class="row-fluid dataTables_wrapper">
                            <form action="/plus/formajax.php" method="post" class="form-horizontal">
								...
								<?php  if(is_array($list)) { foreach($list as $row) { ?>
                                    ...
                                    <td>
									<?php  if($row['m_type']==1) { ?>纯图片式
									<?php  } else if($row['m_type']==2) { ?>带按钮链接
									<?php  } else if($row['m_type']==3) { ?>带按钮分享
									<?php  } else if($row['m_type']==4) { ?>带电话图片
									<?php  } else if($row['m_type']==5) { ?>带导航图片
									<?php  } else { ?>其他模式<?php  } ?></td> 
                                    <td>
